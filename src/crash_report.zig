...
pub const panic = if (build_options.enable_debug_extensions) compilerPanic else std.debug.panicDefault;
pub const panic2 = if (build_options.enable_debug_extensions) compilerPanic else std.debug.panicDefault;
...
pub fn compilerPanic2(cause: std.builtin.Panic.Cause, data: anytype) noreturn {
    @branchHint(.cold);
    PanicSwitch.preDispatch();
    const ret_addr = @returnAddress();
    const stack_ctx: StackContext = .{ .current = .{ .ret_addr = ret_addr } };

    const st: ?*std.builtin.StackTrace = switch (cause) {
        .unwrapped_error, .unwrapped_error_extra => data.st,
        else => null,
    };
    PanicSwitch.dispatch(st, stack_ctx, "");
}
